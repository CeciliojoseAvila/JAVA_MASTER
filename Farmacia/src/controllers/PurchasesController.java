...
public class PurchasesController implements KeyListener, ActionListener, MouseListener {
        ...
        if (e.getSource() == views.btn_add_product_to_buy) {
            ...
            if (getIdSupplier == 0) {
                ...
                if (getIdSupplier != supplier_id) {
                    ...
                    String product_name = views.txt_purchase_name.getText();
                    ...
                    if (amount > 0) {
                        ...
                        for (int i = 0; i < views.purchase_table.getRowCount(); i++) {
                            if (views.purchase_table.getValueAt(i, 1).equals(views.txt_purchase_name.getText())) {
    ...
    private void insertPurchase() {
        ...
        if (purchaseDao.registerPurchaseQuery(getIdSupplier, employee_id, total)) {
            ...
            for (int i = 0; i < views.purchase_table.getRowCount(); i++) {
                int product_id = Integer.parseInt(views.products_table.getValueAt(i, 0).toString());
                int purchase_amount = Integer.parseInt(views.products_table.getValueAt(i, 2).toString());
                double purchase_price = Double.parseDouble(views.products_table.getValueAt(i, 3).toString());
        ...
        if (e.getSource() == views.txt_purchase_product_code) {
            if (e.getKeyCode() == KeyEvent.VK_ENTER) {
                if (views.txt_purchase_product_code.getText().equals("")) {
                    JOptionPane.showMessageDialog(null, "Ingresa el c{odigo del producto a comprar");
                } else {
                    int id = Integer.parseInt(views.txt_purchase_product_code.getText());
                    product = productDao.searchCode(id);
                    views.txt_purchase_name.setText(product.getName());
    ...
    public void limpiarCamposPurchases() {
        views.txt_purchase_name.setText("");
    ...
    public void mouseClicked(MouseEvent e) {
        if (e.getSource() == views.jLabelPurchases) {
            if (rol.equals("Administrador")) {
                views.jTabbedPane2.setSelectedIndex(1);
                cleanTable();
            } else {
                views.jTabbedPane2.setEnabledAt(1, false);
            ...
            views.jTabbedPane2.setSelectedIndex(6);
            cleanTable();
            listAllPurchases();
        }
    }
